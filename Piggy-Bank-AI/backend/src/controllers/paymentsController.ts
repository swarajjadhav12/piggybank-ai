...
export const deposit = async (req: Request, res: Response) => {
  try {
    ...
    // Generate AI tips for the deposit
    try {
      const tips = await tipGenerationService.generateDepositTips(userId, amount);
      await tipGenerationService.storeTipsAsInsights(userId, tips);
    } catch (tipError) {
      console.error('Error generating deposit tips:', tipError);
      // Don't fail the deposit if tip generation fails
    }
